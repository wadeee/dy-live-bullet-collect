...
def interval_rank(roo_id):
    print(f"间隔{config.content['rank_config']['LIVE_RANK_INTERVAL']}秒更新一下排行")
    if config.content['rank_config']['LIVE_RANK_LIST']:
        rank_t = threading.Thread(target=handle_rank, args=(roo_id, config.content['rank_config']['LIVE_RANK_INTERVAL']))
        rank_t.start()
    else:
        print(f"未开启直播礼物排名")
