...
def init_global():
    payload = json.dumps({
        ...
        "gameuuid": config.content['rank_config']['GAME_UUID']
    })
    headers = {
    ...
    try:
        response = requests.request("POST", config.content['rank_config']['LIVE_WEB_SEND_URL'], headers=headers, data=payload)
        query_json = response.json()
        game_data = query_json.get("response_data").get("data")
        for data in game_data:
            if data.get("uuid") == config.content['rank_config']['DONATION_UUID']:
